...
class sinLayer(tf.keras.Model):
    def __init__(self, AT, s_shape=(725, 360), out_size=(512, 512)):
        ...
        w_b = w_bfunction(np.pi,
                          np.linspace(-np.floor(s_shape[0] / 2), s_shape[0] - np.floor(s_shape[0] / 2) - 1, s_shape[0]))
        ...
        for layers in range(1, self.M1 + 1):
            self.sinLayer.append(
                tf.keras.layers.Conv2D(64, 5, padding='same', name='sinconv%d' % layers, use_bias=False))
...
def train(epoch, udir, batch, theta, iternum, restore=0, ckpt='./weights/CT_tf2_4'):
    ...
    f_noisy_img = interp(f_noisy_img, np.linspace(0, 180, angles, endpoint=False),
                         np.linspace(0, 180, 180, endpoint=False))
    ...
    train_data = tf.data.Dataset.from_tensor_slices((u_img[0:N - 5], f_img[0:N - 5], f_noisy_img[0:N - 5])).shuffle(
        tf.cast(N - 5, tf.int64)).batch(batch)
    # _ = Model(vx[0:1])
    if restore == 1:
...
def train_step(inputs, model, labels, Loss, Metric, optimizer, vx, vy, epochnum):
    ...
    m1 = tf.reduce_sum(tf.image.psnr(labels[0], inputs, max_val=tf.reduce_max(labels[0]))) / tf.cast(
        tf.shape(inputs)[0], tf.float32)
