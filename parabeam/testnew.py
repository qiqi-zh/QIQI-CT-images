...
def mm():
    ...
    f_noisy_img = interp(f_noisy_img, np.linspace(0, 180, angles, endpoint=False),
                         np.linspace(0, 180, superangles, endpoint=False))
    L = 500
    print("f_noisy_img:", f_noisy_img.shape[0])
    f_noisy_img = f_noisy_img[0:L]
    f_noisy = tf.cast(f_noisy_img, tf.float32)
    prediction = evaluate(f_noisy, batch, Model, f_noisy_img)
...
def evaluate(f_noisy, batch, Model, f_noisy_img):
...
def printPSNR(n):
    ...
    pp = tf.image.psnr(tf.cast(prediction[n:n+100], tf.float32), vy, tf.reduce_max(prediction[n:n+100])).numpy()
    qq = tf.image.ssim(tf.cast(prediction[n:n+100], tf.float32), vy, tf.reduce_max(prediction[n:n+100])).numpy()
